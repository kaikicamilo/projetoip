...
topic2 = "projeto_fastai_bia_turmaiv"
...
def publish(client):
    ...
    dados = {
        "qtd_pessoas": len(on_ambient),
        "tempo_p": np.mean(tempo_permanencia),
        "tempo_r": np.mean(tempo_recorrencia),
    ...
    def on_message(client, userdata, msg):
        ...
        if len(id_people) == 0:
            if len(face) != 0:
                ...
                on_ambient.append(0)
                gender = find_gender(
                    face.permute(1, 2, 0).cpu().detach().numpy(), gender_m
                )
                ...
                for j in range(len(events), -1, -1):
                    if events[j - 1][0] == ident:
                        ...
                        if dif.total_seconds() >= 15 and events[j - 1][3] == 1:
                            events.append(
                                [ident, atual, 0, 0, 1, dif.total_seconds(), np.nan]
                            )
                            ...
                            events.append(
                                [ident, atual, 0, 1, 0, np.nan, dif.total_seconds()]
                            )
                            ...
                            age = find_age(
                                face.permute(1, 2, 0).cpu().detach().numpy(), age_m
                            )
                            if ident in possiveis_recorrentes:
                                qtd_recorrentes += 1
                            publish(client)
